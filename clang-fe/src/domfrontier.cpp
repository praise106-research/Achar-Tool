...
ssa_transform::domFrontier ssa_transform::DominanceFrontier::getFrontier() { return frontier; }
 ...
 * of the phi function.
 *
 * @param assignments- a map between a variable (given by a string) and the set of CFG blocks where
 * it is assigned
...
ssa_transform::placePhi(std::map<std::string, std::set<clang::CFGBlock*>> assignments,
                ...
                if (hasAlready[cfgBlockY] < itercount) {
                    phiPlacements[cfgBlockY].push_back(
                        std::pair<std::string, std::vector<std::string>>(
                            variable, std::vector<std::string>(cfgBlockY->pred_size() + 1)));
