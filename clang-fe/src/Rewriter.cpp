...
std::string RewriterClass::simplifyNot(clang::Expr* expr, int notLevel = 0) {
    // simplify a condition from !(a && b || c) and the like such that the && and || occur in the
    // root of the condition We use de Morgan's law which states that !(a && b) = !a || !b and !(a
    // || b) = !a && !b
    ...
    if (llvm::isa<UnaryOperator>(expr)) {
        ...
        if (binOp->getOpcodeStr() == "&&") {
            ...
            res.append(simplifyNot(binOp->getLHS()->IgnoreCasts()->IgnoreParens()->IgnoreImplicit(),
                                   notLevel));
            res.append(") ");
            res.append(replacementOp);
            res.append(" (");
            res.append(simplifyNot(binOp->getRHS()->IgnoreCasts()->IgnoreParens()->IgnoreImplicit(),
                                   notLevel));
            res.append(")");
            ...
            if (notLevel % 2) {
                ...
                res.append(simplifyNot(
                    binOp->getLHS()->IgnoreCasts()->IgnoreParens()->IgnoreImplicit(), 0));
                res.append(" ");
                res.append(binOp->getOpcodeStr());
                res.append(" ");
                res.append(simplifyNot(
                    binOp->getRHS()->IgnoreCasts()->IgnoreParens()->IgnoreImplicit(), 0));
                res.append(")");
                return res;
            } else {
                std::string res;
...
bool RewriterClass::VisitStmt(clang::Stmt* s) {
    if (s != nullptr && context->getSourceManager().isInMainFile(s->getBeginLoc())) {
        if (isa<UnaryOperator>(s)) {
            // Either to simplify not statements or to rewrite ++ and -- operators as their
            // simplified assignment operators. eg- x++ -> x = x + 1
            ...
            if (!declStmt->isSingleDecl()) {
                // expand single statement with multiple declarations into multiple statements each
                ...
                for (auto iter = declStmt->decl_begin(); iter != declStmt->decl_end(); iter++) {
                    if (isa<VarDecl>(*iter)) {
                        ...
                        declString +=
                            varDecl->getType().getAsString() + " " + varDecl->getNameAsString();

                        if (varDecl->hasInit()) {
                            auto init = varDecl->getInit();
                            auto initRange =
                                Lexer::getAsCharRange(init->getSourceRange(), sourceManager, lo);
                            auto initEnding = Lexer::getLocForEndOfToken(initRange.getEnd(), 0,
                                                                         sourceManager, lo);
            ...
            if (isa<IntegerLiteral>(whileCond)) {
                ...
                rewriter.InsertText(beginMain, "int cond" + std::to_string(numCond) + " = " +
...
std::unique_ptr<clang::ASTConsumer> RewriterClassAction::CreateASTConsumer(
