 ...
 * General Procedure-
 *
 * Given a c file as an argument, the tool makes 3 passes over the code. The first pass involves
 * Rewriter.h, which rewrites the code so that certain complex statements are simplified and can be
 * used with the remainder of the ssa-transform
 *
 * The second pass involves MainVisitor.h, which visits the statements in the main function and
 * makes several maps which help identify the variables, their locations and their replacements
 *
 * The third pass involves SSAWriter.h which constructs the SSA graph using the maps made by
 * MainVisitor. This SSA graph can then be printed either using the print method or the printAsJSON
 * method.
 *
...
int main(int argc, const char** argv) {
    if (argc > 2) {
        ...
        for (int i = 2; i < argc; i++) {
            ...
            // The only way I could figure out to get variables affected by the Recursive AST
            // Visitor was to pass those variables by reference into the FrontEndAction classes and
            // then retrieve the values when the pass was finished.
