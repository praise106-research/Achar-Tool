...
/**
 * printAsJSON prints the SSAGraph in a JSON format into an outstream (either cout or ofstream if
 * needed to print into a file). Currently every node is printed on a new line, regardless if it has
 * more arguments.
 *
 * @todo Edit the function so that nodes without arguments can be printed on the same line for
 * conciseness.
 *
 * @param indent
 * @param outStream
...
void SSAGraph::generatePrePath(std::vector<std::vector<std::string>>& path, std::string nodeID,
                               std::string terminate, std::set<std::string>& visited) {
    // llvm::errs() << nodeID << "\n";
...
/**
 * A function to clean the SSAGraph after the entire graph is constructed. Involves removing empty
 * nodes that may have been placed, rerouting control flows as needed and changing some nodes as
 * necessary.
 *
 * @todo Remove the function and incorporate the cleaning functionalities into the SSA
 * printSeqCFGStmt.
 */
    ...
    for (const auto& node : nodes) {
        if (node.second->cmdName.empty()) {
            ...
            if (inedgeSrc.size() == 1 && outedgeDest.size() == 2) {
                // this means that src is an the condition inside the if statement represented by
                // the current node and branches out occurs in cases when the if condition is just a
                // function call, like if(__Verifier_nondet_int())
...
std::string SSAGraph::generateSMTCond(std::vector<std::string> path, std::string indent,
...
std::string SSAGraph::getSMT(std::map<std::string, std::set<std::string>>& variables,
    ...
    for (auto var : usedVars) {
        ...
        if (smtMode == "-smt") {
            smt += "( declare-const " + var + " (_ BitVec 32) )\n";
            smt += "( declare-const " + var + "! (_ BitVec 32) )\n";
        } else {
            smt += "( declare-primed-var " + var + " (_ BitVec 32) )\n";
        }
    ...
    if (tmpNeeded) {
        if (smtMode == "-smt") {
            smt += "( declare-const " + tmpVar + " (_ BitVec 32) )\n";
            smt += "( declare-const " + tmpVar + "! (_ BitVec 32) )\n";
        } else {
            smt += "( declare-primed-var " + tmpVar + " (_ BitVec 32) )\n";
        }
    ...
    for (auto var : usedVars) {
        for (auto ssVar : variables[var]) {
            if (smtMode == "-smt") {
                smt += "( declare-const " + ssVar + " (_ BitVec 32) )\n";
            } else {
                smt += "( declare-primed-var " + ssVar + " (_ BitVec 32) )\n";
            }
    ...
    for (int i = 0; i < path.size() - 1; i++) {
        ...
        for (auto node : path[i]) {
            ...
            if (assvar != "") assVar.insert(assvar.substr(0, assvar.find_last_of('_')));
        }
        ...
        std::set_difference(usedVars.begin(), usedVars.end(), assVar.begin(), assVar.end(),
                            std::inserter(unrefVar, unrefVar.end()));
