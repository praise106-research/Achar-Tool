...
/**
 * This constructs a SubSSANode depending on the expression type and the number and types of
 * children for the expr
 * @param e
...
/**
 * This constructs a SSANode depending on the expression type and the number and types of children
 * for the expr
 * @param e
...
std::unique_ptr<SSANode> SSAWriterVisitor::getSSANode(clang::Stmt* s) {
    ...
    if (llvm::isa<BinaryOperator>(s)) {
        ...
        if (std::string(castExpr->getCastKindName()).find("ToBoolean") != std::string::npos) {
            // find parent and see if it is either an if condition or it is a condition of a while
            // or for loop
...
/**
 * For a CFGBlock block, this function iterates over each CFGElement, gets them as statements and
 * constructs the appropriate SSANode for them and adds them to the SSAGraph along with the proper
 * control flow.
 *
 * @todo Change this function to accept a Dominator Tree node, since we can get the CFGBlock from
 * the Dominator Tree and also that the Dominator tree does not have loops, which will make the
 * visited vector unnecessary
 *
 * @param block
 * @param visited a boolean vector. visited[blockID] denotes if this block has been previously
 * visited
 */
void SSAWriterVisitor::visitSeqCFGStmt(clang::CFGBlock* block, std::vector<bool>& visited) {
    if (!visited.at(block->getBlockID())) {
        ...
        if (block->pred_empty()) {
            ...
            // The following loop gets all the control flows to the node ID and replaces the flow
            // destination with the EXIT id
            ...
            if (!block->empty()) {
                ...
                // Iterate over all CFGElements of block, get them as Statements and then process
                // each one
                for (auto& I : *block) {
                    if (Optional<CFGStmt> CS = I.getAs<CFGStmt>()) {
                        ...
                        if (ssaNode != nullptr) {
                                ...
                                if (block->pred_size() > 1) {
                                    ...
                                    if (residentLoop.first == 0) {
                                        ...
                                        if (isWithin(const_cast<Stmt*>(s), forStmt->getCond())) {
                                            ...
                                            loopNode->id = std::to_string(block->getBlockID()) +
                                                           "_" + std::to_string(stmtNo);
                            ...
                            if (stmtNo < newSizeOfBlock) {
                                ...
                                ssaGraph.addFlow(ssaNode->id, std::to_string(block->getBlockID()) +
                                                                  "_" + std::to_string(stmtNo + 1));
                                ...
                                if (block->succ_size() == 1) {
                                    ...
                                    // this shows that the block has an if condition and must split
                                    // into two branches a true and a false branch
                            ...
                            // ssaNode will be a nullpointer in events that do not concern the SSA,
                            // like declarations llvm::errs() << "No node of block " <<
                            // block->getBlockID() << " and stmt " << stmtNo << " to graph\n";
                            if (stmtNo == newSizeOfBlock) {
                                // if last statement
                                // we now need to edit the destination nodes of all the predecessors
                                // to this CFGElement we get the predecessors by checking which
                                // control flows point to the current stmtNo
                                ...
                                // that their control goes to the first statement of the next block,
                                // or EXIT if the next case is the exit block
                                ...
                                for (auto succ : block->succs()) {
                                    for (auto src : srcList) {
                                        if (ssaGraph.isExitNode(std::to_string(succ->getBlockID()) +
                                            ...
                                            ssaGraph.addFlow(
                                                src, std::to_string(succ->getBlockID()) + "_1");
                                        }
                                    }
                                }
                ...
                // Here the block is completely empty, signifying that this is probably a block
                // where two branches merge, and so we must take care of any phi functions that may
                // occur in this block
                ...
                if (phiPlacementMap[block->getBlockID()].empty()) {
                    ...
                    // We simply redirect the control flows of the block's predecessors to the
                    // block's successors
