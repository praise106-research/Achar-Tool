...
class MainFunctionVisitor : public clang::RecursiveASTVisitor<MainFunctionVisitor> {
   public:
    ...
    std::map<std::string, std::set<std::string>> getVariableMap() { return variables; }

   private:
    clang::ASTContext* Context;

    /**
     * varStmtStruct- This is a structure which contains the information about a variable. Each
     * occurrence of a variable in the parsed program will have an instantiation of the structure
     * associated with it. The structure has info about the original variable name, the statement it
     * occurs in and the location of the variable.
     */
    struct varStmtStruct {
        ...
        varStmtStruct(std::string varName, clang::Stmt* varStmt, clang::SourceLocation varBegin,
    ...
    // a map between a variable and the list of CFGBlocks where that variable is assigned a new
    // value used for placing phi functions and renaming
    ...
    std::map<clang::CFGBlock*, std::list<std::pair<std::string, std::vector<std::string>>>>
        phiPlacements;

    // a redundant datastructure which has to be replaced- a map between CFGBlock* does not hold
    // true after the pass is completed, so to transfer the information from MainVisitor to
    // SSAWriter, I had to map the block id to the set of phi functions
...
class MainClassAction : public clang::ASTFrontendAction {
   ...
   public:
    ...
    void setPhiPlaceMap(
        std::map<int, std::list<std::pair<std::string, std::vector<std::string>>>>& map) {
        phiPlacementMap = &map;
    }

    void setVariableMap(std::map<std::string, std::set<std::string>>& map) { variableMap = &map; }
