 ...
 *
 * SSANode- a structure which contains the statement command (assign, assert, if, etc.) and the
 * arguments to that statement.
 *
 * SSASubNode- the arguments of SSANode are given by SSASubnodes, which consist of a parent, type of
 * node and a list of arguments.
 */
...
class SSAGraph {
   private:
    ...
    void generatePrePath(std::vector<std::vector<std::string>>& path, std::string nodeID,
                         std::string terminate, std::set<std::string>& visited);
    std::string generateSMTCond(std::vector<std::string> path, std::string indent,
   ...
   public:
    void addNode(std::unique_ptr<SSANode>& node) { nodes[node->id] = (std::move(node)); }
    ...
    std::set<std::pair<std::string, std::string>>& getControlFlow() { return control_flow; }

    void eraseCtrlFlow(std::pair<std::string, std::string> x) { control_flow.erase(x); }
    ...
    /**
     * A function to clean the SSAGraph after the entire graph is constructed. Involves removing
     * empty nodes that may have been placed, rerouting control flows as needed and changing some
     * nodes as necessary. Should be removed when the placement function is optimized enough to
     * avoid such placement errors.
     */
    void clean();

    /**
     * A function to simplify the control flow of the SSA Graph in accordance to the simplification
     * provided by Sparrow
