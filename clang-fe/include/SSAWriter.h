...
class SSAWriterVisitor : public RecursiveASTVisitor<SSAWriterVisitor> {
   public:
    explicit SSAWriterVisitor(
        ...
        : Context(Context),
          varReplacementMap(std::move(varRepmap)),
          phiPlacementMap(phiPlaceMap),
          variableMap(std::move(varMap)),
          srcFile(std::move(filename)),
          genmode(std::move(mode)) {
   ...
   private:
    ...
    /**
     * Checks if a statement is in a loop. If so, return the parent loop statment (either ForStmt,
     * WhileStmt or DoStmt) and a number corresponding to the parent statement (-1 No loop, 0
     * ForStmt, 1 WhileStmt, 2 DoStmt)
     * @param s
     */
     ...
     *
     * @todo Try and optimize the placement of the SSANodes to avoid the need of the
     * SSAGraph::clean() function
     *
     * @param block
     * @param visited
     */
   ...
   public:
    explicit SSAWriterConsumer(
        ...
        :
          ...
          Visitor(Context, varRepMap, phiPlaceMap, std::move(varMap), std::move(filename),
                  std::move(mode)) {}
   ...
   public:
    ...
    virtual std::unique_ptr<clang::ASTConsumer> CreateASTConsumer(clang::CompilerInstance& Compiler,
        ...
        return std::unique_ptr<clang::ASTConsumer>(
            new SSAWriterConsumer(&Compiler.getASTContext(), varReplacementMap, phiPlacementMap,
                                  variableMap, filename, mode));
    }
};
