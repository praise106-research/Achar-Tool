...
class RewriterClass : public clang::RecursiveASTVisitor<RewriterClass> {
   private:
    ...
    /**
     * simplifyNot- A function to simplify conditions surrounded by not, like !(c1 && c2) which
     * interferes with the SSAWriter's functioning
     *
     * @param expr
     * @param notLevel
   ...
   public:
    ...
    bool VisitFunctionDecl(clang::FunctionDecl* d) {
        ...
        if (d->isMain()) {
            ...
            rewriter.InsertText(d->getBeginLoc(),
...
class RewriterConsumer : public clang::ASTConsumer {
   public:
    explicit RewriterConsumer(clang::ASTContext* Context, clang::Rewriter& rewriter)
