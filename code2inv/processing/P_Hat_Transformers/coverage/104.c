...
void precheck(FILE* fptr, char* modelFileBuffer, long long int n, long long int x, long long int v1,
              long long int v2, long long int v3) {
    int f = preflag;
    setflag(INV(n, x, v1, v2, v3), preflag);
    if (f == 0 && preflag == 1) {
...
void loopcheck(FILE* fptr, char* modelFileBuffer, long long int temp_n, long long int temp_x,
               long long int temp_v1, long long int temp_v2, long long int temp_v3, long long int n,
               long long int x, long long int v1, long long int v2, long long int v3) {
    int f = loopflag;
    setflag(INV(n, x, v1, v2, v3), loopflag);
    if (f == 0 && loopflag == 1) {
        fprintf(fptr, "LoopStart : %s : %lld, %s : %lld, %s : %lld, %s : %lld, %s : %lld\n", "n",
                temp_n, "x", temp_x, "v1", temp_v1, "v2", temp_v2, "v3", temp_v3);
        fprintf(fptr, "LoopEnd : %s : %lld, %s : %lld, %s : %lld, %s : %lld, %s : %lld\n", "n", n,
                "x", x, "v1", v1, "v2", v2, "v3", v3);
...
#define postcheck(fptr, modelFileBuffer, cond, n, x, v1, v2, v3) \
    \ 
{                                                           \
        \ 
    int f = postflag;                                            \
...
int main(int argc, char** argv) {
    ...
    // Translate()
    if (choices > 10000) {
        prebody_f1(choices, n, x, v1, v2, v3);

        precount++;
        // fprintf(fptr, "Pre\n");
        // precheck
        precheck(fptr, vars, n, x, v1, v2, v3);
    } else {
        // loop-check program
        assume((loopflag + postflag < 2));
        assume(INV(n, x, v1, v2, v3));
        ...
        if ((x < n)) {
            ...
            while ((x < n) && unroll--) {
                ...
                loopbody_f2(choices, n, x, v1, v2, v3);

                loopcount++;
                loopcheck(fptr, vars, temp_n, temp_x, temp_v1, temp_v2, temp_v3, n, x, v1, v2, v3);
            }
        } else {
            // post-check program
            assume((postflag == 0));
            ...
            // post-condition
            if ((x != n)) {
                postcheck(fptr, vars, ((n < 0)), n, x, v1, v2, v3)
            }
        }
    }
