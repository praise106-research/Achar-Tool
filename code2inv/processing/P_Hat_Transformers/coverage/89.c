...
int main(int argc, char** argv) {
    ...
    // variable declarations
    long long int lock = atoi(argv[1]);
    ...
    // Translate()
    if (choices > 10000) {
        prebody_f1(choices, lock, x, y, v1, v2, v3);
        ...
        if ((x != y)) {
            ...
            while ((x != y) && unroll--) {
                ...
                loopbody_f2(choices, lock, x, y, v1, v2, v3);

                loopcount++;
                loopcheck(fptr, vars, temp_lock, temp_x, temp_y, temp_v1, temp_v2, temp_v3, lock, x,
                          y, v1, v2, v3);
            }
        } else {
            // post-check program
