#!/usr/bin/python3

# We manually annotate the code as per the template below and then
# we can run the code to generate P-Hat Trasnformation for Colossus

import sys
import os

abc = sys.argv[1].strip()
fileName = sys.argv[2].strip()

if ";" in abc:
    varList = [str(v.strip()) for v in sys.argv[1].strip().split(";")]
else:
    varList = [str(v.strip()) for v in sys.argv[1].strip().split()]

while "" in varList:
    varList.remove("")


def pre(arglist="", formatString="", arglistString="", varlistString=""):
    PreTemplate = "// COMMENT : Precheck function \n"
    PreTemplate += f"void precheck({arglist})\n"
    PreTemplate += "{\n"
    PreTemplate += "    int f = preflag;"
    PreTemplate += f"   setflag(INV({arglistString}), preflag); \n"
    PreTemplate += "    if (f == 0 && preflag == 1){\n"
    PreTemplate += (
        f'       printf( "Pre : {formatString}\\n", \n {varlistString});\n')
    PreTemplate += "    /* assert(0); */ \n}}\n"
    return PreTemplate


def loop(
    arglist="",
    formatString="",
    arglistString="",
    varlistString="",
    looptemplist="",
    loopargsString="",
):
    loopTemplate = "// COMMENT : Loopcheck function \n"
    loopTemplate += f"void loopcheck({looptemplist}, {arglist})\n"
    loopTemplate += "{\n"
    loopTemplate += "    int f = loopflag;\n"
    loopTemplate += f"   setflag(INV({arglistString}), loopflag); \n"
    loopTemplate += "    if (f == 0 && loopflag == 1){\n"
    loopTemplate += (
        f'       printf("LoopStart : {formatString}\\n", \n {loopargsString});\n'
    )
    loopTemplate += (
        f'       printf("LoopEnd : {formatString}\\n", \n {varlistString});\n')
    loopTemplate += "   /* assert(0); */\n}}\n"
    return loopTemplate


def post(arglist="", formatString="", arglistString="", varlistString=""):
    postTemplate = "// COMMENT : Postcheck Macro \n"
    postTemplate += (f"#define postcheck(cond, {arglistString})\ \n")
    postTemplate += "{\ \n"
    postTemplate += "    int f = postflag;\ \n"
    postTemplate += f"   setflag(cond, postflag);\ \n"
    postTemplate += "    if (f == 0 && postflag == 1){\ \n"
    postTemplate += f'        printf("Post : {formatString}\\n", \ \n {varlistString}); /* assert(0); */ \ \n'
    postTemplate += "}}\n"
    return postTemplate


pwd = os.path.dirname(__file__)
sourcepath = sys.argv[3]

# Read in the source code for transformation.
sourcefile = os.path.join(sourcepath, f"{fileName}.c")
includefile = os.path.join(os.pardir, f"include/loop_{fileName}.h")

if __name__ == "__main__":
    arglist = ""
    varlistString = ""
    formatString = ""
    arglistString = ""
    looptemplist = ""
    loopargsString = ""
    loopvarListstr = ""
    loopTermTerms = ""

    with open(sourcefile, mode="r") as fileptr:
        code = fileptr.readlines()

    with open(includefile, mode="w") as fileptr:
        fileptr.write(f"#define PHI 1 // generated by code2inv \n")

    for v in varList:
        varlistString += f"{v}, "
        loopvarListstr += f"temp_{v}, "
        arglist += f"int {v}, "
        looptemplist += f"int temp_{v}, "
        formatString += "%s : %d, "
        arglistString += f'"{v}", {v}, '
        loopargsString += f'"{v}", temp_{v}, '
        loopTermTerms += f"int temp_{v} = {v};\n"

    HonggFuzzInputLines = []
    variablesDecl = []
    preconditions = []
    loopbody = []
    postcondition = []
    preMainText = []
    condString = "(choices > 2500)"

    index = 0
    scanf = 0
    lines = code[index]

    for r, line in enumerate(code):
        if "int main()" in line.strip():
            index = r
            break
        preMainText.append(line.strip())

    lines = code[index]

    while index < len(code):
        lines = lines.strip()

        if "/*" in lines:
            while "*/" not in lines:
                index = index + 1
                lines = code[index]

        if "*/" in lines:
            index = index + 1
            lines = code[index]

        if "int main()" in lines:
            index = index + 1
            lines = code[index]

        if "variable declarations" in lines:
            while "pre-conditions" not in lines:
                lines = f"{lines.strip()}"
                variablesDecl.append(lines)
                index = index + 1
                lines = code[index]

        if "pre-conditions" in lines:
            while "precheck" not in lines:
                if "scanf" in lines:
                    scanf += 1
                    lines = lines.strip()
                    lines = f"klee_make_symbolic(&{lines.split('&')[-1].strip()[0:-2]}, sizeof({lines.split('&')[-1].strip()[0:-2]}), \"{lines.split('&')[-1].strip()[0:-2]}_sym\");"
                    HonggFuzzInputLines.append(lines)
                    lines = ""
                if "assume" in lines:
                    lines = lines.strip()
                    lines = f"klee_{lines}"
                preconditions.append(lines)
                index = index + 1
                lines = code[index]

        if "precheck" in lines:
            index = index + 1
            lines = code[index]

        if "loopcond" in lines:
            condString = lines.split(":")[-1].strip()
            if "unknown" in condString:
                condString = "(choices > 2500)"
            index = index + 1
            lines = code[index]

        if "loopstart" in lines:
            index = index + 1
            lines = code[index]

        if "while" in lines:
            index = index + 1
            lines = code[index]
            loopbody.append("{ \n")

        if "loop body" in lines:
            while "loopend" not in lines:
                if "unknown" in lines:
                    lines = "// non-deterministic \n if (choices > 4500) {"
                loopbody.append(lines)
                index = index + 1
                lines = code[index]

        if "loopend" in lines:
            index = index + 1
            lines = code[index]

        if "postcheck" in lines:
            index = index + 1
            lines = code[index]

        if "post-condition" in lines:
            while index < len(code) - 1:
                if "assert" in lines:
                    assertCond = lines.split("assert")[-1].strip()
                    lines = f"""
                                postcheck({assertCond.strip()[:-1]}, {varlistString.strip()[:-1]})
                            """
                postcondition.append(lines.strip())
                index = index + 1
                lines = code[index]

        index = index + 1

    # if len(HonggFuzzInputLines) <= 1 and len(varList) >= 2:
    #     for variable in varList:
    #         scanf += 1
    #         lines = f"long long int {variable} = honggBuf[{scanf}];"
    #         HonggFuzzInputLines.append(lines)

    newline = "\n"
    preconditions.pop(0)
    variablesDecl.pop(0)
    loopbody = f"\n{newline.join([x.strip() for x in loopbody])}"
    prebody = f"\n{newline.join([x.strip() for x in preconditions])}"
    HonggFuzzInputLinesSTR = f"{newline.join([x.strip() for x in HonggFuzzInputLines])}"
    postbody = f"\n{newline.join([x.strip() for x in postcondition])}"
    preMainBody = f"\n{newline.join([x.strip() for x in preMainText])}"
    valDecl = f"\n{newline.join([x.strip() for x in variablesDecl])}"

    K = 1
    template = f"""
        /*
        * P-Hat Transformation 
        * for Colossus : {fileName}.c file
        */

        # include <loop_{fileName}.h>
        #include <assert.h>
        #include <klee/klee.h>
        #include <stdarg.h>
        #include <stdio.h>
        #include <limits.h>
        #include <stdlib.h>
        #include <string.h>
        #include <sys/file.h>
        
        {preMainBody}
        
        # define UNROLL_LIMIT {K}
        
        # define setflag(cond, flag)                                                   \
        if (!cond)                                                                   \
            flag = 1;

        # define assume(cond)                                                           \
        if (!cond)                                                                   \
            exit(0);

        # define INV({varlistString.strip()[:-1]}) PHI

        long long unsigned int counter = 0;
        int preflag = 0, loopflag = 0, postflag = 0;
        long long unsigned int precount = 0, loopcount = 0, postcount = 0;

        {pre(arglist.strip()[:-1],
                formatString.strip()[:-1],
                varlistString.strip()[:-1],
                arglistString.strip()[:-1])}
        
        {loop(arglist.strip()[:-1],
                formatString.strip()[:-1],
                varlistString.strip()[:-1],
                arglistString.strip()[:-1],
                looptemplist.strip()[:-1],
                loopargsString.strip()[:-1])}
        
        {post(arglist.strip()[:-1],
                formatString.strip()[:-1],
                varlistString.strip()[:-1],
                arglistString.strip()[:-1])}

        int main() {{
            // variable declarations
            {valDecl}
            int choices;
            // klee make symbolic calls.
            {HonggFuzzInputLinesSTR}
            klee_make_symbolic(&choices, sizeof(choices), "choices_sym");

            // Translate() Colossus
            if (choices > 10000) {{
                {prebody}
                precount++;
                // fprintf(fptr, "Pre\\n");
                // precheck
                precheck({varlistString.strip()[:-1]});
            }} else {{
                // loop-check program
                // assume((loopflag + postflag < 2));
                klee_assume(INV({varlistString.strip()[:-1]}));
                // Loop Condition
                if ({condString}) {{
                    // K-Bounded Exploration
                    int unroll = UNROLL_LIMIT;
                    while ({condString} && unroll--) {{
                        // assume((loopflag == 0));
                            {loopTermTerms}
                            {loopbody}
                        // fprintf(fptr, "Loop\\n");
                        // fprintf(fptr, "Flag : %d\\n", reach_flag);
                        // assume(reach_flag);
                        
                        loopcount++;
                        loopcheck(
                            {loopvarListstr.strip()[:-1]}, 
                            {varlistString.strip()[:-1]});
                    }}
                }} else {{
                    // post-check program
                    // assume((postflag == 0));
                    postcount++;
                    // fprintf(fptr, "Post\\n");
                    {postbody}
                }}
            }}

            // Print the counters if no flags are hit
            if (preflag + loopflag + postflag == 0 && counter == 100) {{
                printf("%s : %lld, %s : %lld, %s : %lld[newline]", 
                    "precount", precount, "loopcount", loopcount, "postcount", postcount);
                counter = 0;
            }}

            // Regular Close FILE 
            return 0;
        }}
    """
    print(template.replace("[newline]", "\\n").replace("\0", "\\0"))
