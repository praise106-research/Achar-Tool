...
class StatsCounter(object):
    ...
    def report_str(self, pid):
        ...
        report += 'z3_report time: %.2f pid: %s stats: %s' % (dur, str(pid),
                                                              str(c))
...
class CounterExample(object):
    ...
    def to_ice_str(self):
        ...
        if self.kind == "pre":
            ...
            res = "I:{" + ",".join([
                "%s=%s" % (k, self.config[0][k])
                for k in sorted(self.config[0].keys())
            ]) + ";"
            res += ",".join([
                "%s=%s" % (k, self.config[1][k])
    ...
    def helper(self, dat, expr_root):
        try:
            ...
            r = checker_module.inv_checker(cmd_args.input_vcs, str(expr_root),
                                           assignments)
    ...
    try:
        ...
        if cmd_args.inv_reward_type == 'any':
            ...
            if sys.version_info[0] < 3:
                out = check_output([
                    "mono", cmd_args.boogie_exe, "-noinfer", "-mlHoudini:ice",
                    s3
                ])
            else:
                out = check_output([
...
def report_tested_stats(g, roots):
    ...
    arr = np.array(stats)
    print("mean: ", np.mean(arr), " std: ", np.std(arr), "min: ", np.min(arr),
          "max: ", np.max(arr), "median: ", np.median(arr))
...
def boogie_result(g, expr_root):
    ...
    def lambda_holder_eval(key):
        return holder.eval(key, expr_root)

    if cmd_args.only_use_z3:
        if cmd_args.single_sample is not None:

            def lambda_new_ice():
                return get_z3_ice(g.db.ordered_pre_post[g.sample_index],
                                  expr_root)
        else:

            def lambda_new_ice():
                return get_z3_ice(g.vc_list, expr_root)
    else:
        if cmd_args.single_sample is not None:

            def lambda_new_ice():
                return get_boogie_ice(g.db.ordered_pre_post[g.sample_index],
                                      expr_root)
        else:

            def lambda_new_ice():
                return get_boogie_ice(g.vc_list, expr_root)

    res = reward_1(g.sample_index, holder, lambda_holder_eval, lambda_new_ice)

    if res > 0:
        tqdm.write("found a solution for " + str(g.sample_index) + " , sol: " +
                   str(expr_root))
