...
class ExprNode(object):
    ...
    def __eq__(self, othernode):
        if othernode is None:
            return False
        res = (self.name == othernode.name) and (
            self.pg_node == othernode.pg_node) and (
                self.children == othernode.children)
        return res
        ...
        if len(self.children) > 0:
            ...
            if op == "&&":
                ...
                if len(self.children) == 3 and self.children[1].name in (
                        "<", ">", "==", "<=", ">=", "+", "-"):
                    return "(%s %s)" % (self.children[1].name, " ".join([
                        self.children[0].to_smt2(), self.children[2].to_smt2()
                    ]))
            return "(%s %s)" % (op, " ".join(
                [c.to_smt2() for c in self.children]))
        return self.name
    ...
    def to_z3(self):
        ...
        if self.name == "&&":
            ...
            return "(%s %s %s)" % (child_list[0].to_z3(), self.name,
                                   child_list[1].to_z3())
        else:
    ...
    def get_vars(self, st):
        child_list = self.children
        if len(child_list) == 0:
            if self.name.startswith('-') or self.name.isdigit(
            ) or self.name in ("<", ">", "==", "<=", ">=", "+", "-"):
                pass
            else:
                st.add(self.name)
    ...
    def to_py(self):
        ...
        if self.name == "&&":
            ...
            return "( " + child_list[0].to_py(
            ) + " " + self.name + " " + child_list[1].to_py() + " )"
            # return  "(%s %s %s)" % (self.children[0].to_z3(), self.name, self.children[1].to_z3() )
        else:
            if self.name == "" and len(
                    self.children) == 3 and self.children[1].name in (
                        "<", ">", "==", "<=", ">=", "+", "-"):
                return "( " + self.children[0].to_py() + " " + self.children[
                    1].name + " " + self.children[2].to_py() + " )"
            assert len(child_list) == 0
            return self.name
...
class ProgramGraph(object):
    def __init__(self, graph_json):
        ...
        if cmd_args.encoder_model != "Param":
            ...
            for edges in graph_json['control-flow']:
                assert edges[0] in self.semantic_nodes and edges[
                    1] in self.semantic_nodes
                ...
                self.add_double_dir_edge(x, y, CONTROL_EDGE_TYPE,
                                         CONTROL_EDGE_TYPE + 1)
            ...
            self.assert_statement = self.build_assert(val['rval'],
                                                      self.core_vars)
    ...
    def add_ssa_variables(self, d):
        ...
        if 'Var' in d:
            ...
            self.add_double_dir_edge(ssa_node.index, var_node.index,
                                     VAR_LINK_TYPE, VAR_LINK_TYPE + 1)
        else:
            for key in d:
    ...
    def traverse_ast(self, node, d):
        if 'Var' in d:  # var node
            ...
            self.add_double_dir_edge(node.index, var_node.index, AST_EDGE_TYPE,
                                     AST_EDGE_TYPE + 1)
        elif 'Const' in d:  # const node
            assert len(d) == 1  # const node
            const_node = self.const_nodes[d['Const']]
            self.add_double_dir_edge(node.index, const_node.index,
                                     AST_EDGE_TYPE, AST_EDGE_TYPE + 1)
        elif 'UNK' in d:  # unk node
            assert len(d) == 1  # unk node
            self.add_double_dir_edge(node.index, self.unk_node.index,
                                     AST_EDGE_TYPE, AST_EDGE_TYPE + 1)
        else:
            ...
            for key in d:
                if type(d[key]) is dict:
                    ...
                    self.add_double_dir_edge(node.index, child.index,
                                             AST_EDGE_TYPE, AST_EDGE_TYPE + 1)
    ...
    def add_double_dir_edge(self, src_idx, dst_idx, edge_type_forward,
                            edge_type_backward):
