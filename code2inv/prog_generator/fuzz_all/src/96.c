...
void precheck(FILE* file_descp, char* buff, long long int i, long long int j, long long int x,
              long long int y) {
    int f = preflag;
...
int main() {
    ...
    for (;;) {
        ...
        snprintf(vars, 128, "%s : %lld, %s : %lld, %s : %lld, %s : %lld", "i", i, "j", j, "x", x,
                 "y", y);
