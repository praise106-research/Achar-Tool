...
void loopcheck(int temp_x, int temp_size, int temp_y, int temp_z, int x, int size, int y, int z) {
    int f = loopflag;
    setflag(INV(x, size, y, z), loopflag);
    if (f == 0 && loopflag == 1) {
        printf("LoopStart : %s : %d, %s : %d, %s : %d, %s : %d\n", "x", temp_x, "size", temp_size,
               "y", temp_y, "z", temp_z);
    ...
    if (choices > 10000) {
        ...
        if ((x < size)) {
            ...
            while ((x < size) && unroll--) {
                ...
                loopcount++;
                loopcheck(temp_x, temp_size, temp_y, temp_z, x, size, y, z);
            }
        } else {
            // post-check program
            // assume((postflag == 0));
