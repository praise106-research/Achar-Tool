...
void precheck(int v1, int v2, int v3, int x, int size, int y, int z) {
    int f = preflag;
    setflag(INV(v1, v2, v3, x, size, y, z), preflag);
    if (f == 0 && preflag == 1) {
        printf("Pre : %s : %d, %s : %d, %s : %d, %s : %d, %s : %d, %s : %d, %s : %d\n", "v1", v1,
               "v2", v2, "v3", v3, "x", x, "size", size, "y", y, "z", z);
        /* assert(0); */
    }
}

// COMMENT : Loopcheck function
void loopcheck(int temp_v1, int temp_v2, int temp_v3, int temp_x, int temp_size, int temp_y,
               int temp_z, int v1, int v2, int v3, int x, int size, int y, int z) {
    int f = loopflag;
...
#define postcheck(cond, v1, v2, v3, x, size, y, z)                                             \
    \ 
{                                                                                         \
        \ 
    int f = postflag;                                                                          \
 ...
 "v1",                                                                                         \
               v1, "v2", v2, "v3", v3, "x", x, "size", size, "y", y, "z", z); /* assert(0); */ \
        \ 
}                                                                                     \
    }
...
int main(int argc, char* argv[]) {
    ...
    if (choices > 10000) {
        ...
        // fprintf(fptr, "Pre\n");
        // precheck
        precheck(v1, v2, v3, x, size, y, z);
    } else {
        // loop-check program
        // assume((loopflag + postflag < 2));
        klee_assume(INV(v1, v2, v3, x, size, y, z));
        // Loop Condition
        if ((x < size)) {
            ...
            while ((x < size) && unroll--) {
                ...
                loopcount++;
                loopcheck(temp_v1, temp_v2, temp_v3, temp_x, temp_size, temp_y, temp_z, v1, v2, v3,
                          x, size, y, z);
            }
        } else {
            // post-check program
            ...
            // post-condition
            if (size > 0) {
                postcheck((z >= y), v1, v2, v3, x, size, y, z)
            }
        }
    }
