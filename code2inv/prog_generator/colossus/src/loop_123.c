...
void precheck(int i, int size, int sn, int v1, int v2, int v3) {
    int f = preflag;
...
void loopcheck(int temp_i, int temp_size, int temp_sn, int temp_v1, int temp_v2, int temp_v3, int i,
               int size, int sn, int v1, int v2, int v3) {
    int f = loopflag;
    setflag(INV(i, size, sn, v1, v2, v3), loopflag);
    if (f == 0 && loopflag == 1) {
        printf("LoopStart : %s : %d, %s : %d, %s : %d, %s : %d, %s : %d, %s : %d\n", "i", temp_i,
               "size", temp_size, "sn", temp_sn, "v1", temp_v1, "v2", temp_v2, "v3", temp_v3);
        printf("LoopEnd : %s : %d, %s : %d, %s : %d, %s : %d, %s : %d, %s : %d\n", "i", i, "size",
               size, "sn", sn, "v1", v1, "v2", v2, "v3", v3);
        /* assert(0); */
    }
}

// COMMENT : Postcheck Macro
#define postcheck(cond, i, size, sn, v1, v2, v3)                                          \
    \ 
{                                                                                    \
        \ 
    int f = postflag;                                                                     \
 ...
 "i",                                                                                     \
               i, "size", size, "sn", sn, "v1", v1, "v2", v2, "v3", v3); /* assert(0); */ \
        \ 
}                                                                                \
    }
...
int main(int argc, char* argv[]) {
    ...
    if (choices > 10000) {
        ...
        // fprintf(fptr, "Pre\n");
        // precheck
        precheck(i, size, sn, v1, v2, v3);
    } else {
        // loop-check program
        // assume((loopflag + postflag < 2));
        klee_assume(INV(i, size, sn, v1, v2, v3));
        // Loop Condition
        if ((i <= size)) {
            ...
            while ((i <= size) && unroll--) {
                ...
                loopcount++;
                loopcheck(temp_i, temp_size, temp_sn, temp_v1, temp_v2, temp_v3, i, size, sn, v1,
                          v2, v3);
            }
        } else {
            // post-check program
            ...
            // post-condition
            if ((sn != 0)) postcheck(((sn == size)), i, size, sn, v1, v2, v3)
        }
    }
