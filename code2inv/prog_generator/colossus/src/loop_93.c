...
void precheck(int i, int n, int x, int y) {
    int f = preflag;
    setflag(INV(i, n, x, y), preflag);
    if (f == 0 && preflag == 1) {
...
void loopcheck(int temp_i, int temp_n, int temp_x, int temp_y, int i, int n, int x, int y) {
    int f = loopflag;
    setflag(INV(i, n, x, y), loopflag);
    if (f == 0 && loopflag == 1) {
    ...
    if (choices > 10000) {
        ...
        if ((i < n)) {
            ...
            while ((i < n) && unroll--) {
                ...
                loopcount++;
                loopcheck(temp_i, temp_n, temp_x, temp_y, i, n, x, y);
            }
        } else {
            // post-check program
            // assume((postflag == 0));
