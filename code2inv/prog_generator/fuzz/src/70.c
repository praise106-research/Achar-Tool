...
void precheck(FILE* fptr, char* modelFileBuffer, long long int x, long long int y, long long int n,
              long long int v1, long long int v2, long long int v3) {
    int f = preflag;
    setflag(INV(x, y, n, v1, v2, v3), preflag);
    if (f == 0 && preflag == 1) {
        fprintf(fptr, "Pre : %s : %lld, %s : %lld, %s : %lld, %s : %lld, %s : %lld, %s : %lld\n",
                "x", x, "y", y, "n", n, "v1", v1, "v2", v2, "v3", v3);
...
void loopcheck(FILE* fptr, char* modelFileBuffer, long long int temp_x, long long int temp_y,
               long long int temp_n, long long int temp_v1, long long int temp_v2,
               long long int temp_v3, long long int x, long long int y, long long int n,
               long long int v1, long long int v2, long long int v3) {
    int f = loopflag;
    setflag(INV(x, y, n, v1, v2, v3), loopflag);
    if (f == 0 && loopflag == 1) {
        fprintf(fptr,
                "LoopStart : %s : %lld, %s : %lld, %s : %lld, %s : %lld, %s : %lld, %s : %lld\n",
                "x", temp_x, "y", temp_y, "n", temp_n, "v1", temp_v1, "v2", temp_v2, "v3", temp_v3);
        fprintf(fptr,
                "LoopEnd : %s : %lld, %s : %lld, %s : %lld, %s : %lld, %s : %lld, %s : %lld\n", "x",
                x, "y", y, "n", n, "v1", v1, "v2", v2, "v3", v3);
...
#define postcheck(fptr, modelFileBuffer, cond, x, y, n, v1, v2, v3) \
    \ 
{                                                              \
        \ 
    int f = postflag;                                               \
...
// variable declarations
    ...
    FILE* inputs = fopen("inputs/inputs_70.txt", "a");
    ...
    for (;;) {
        ...
        if (choices > 10000) {
            ...
            // fprintf(fptr, "Pre\n");
            // precheck
            precheck(fptr, vars, x, y, n, v1, v2, v3);
        } else {
            // loop-check program
            assume((loopflag + postflag < 2));
            assume(INV(x, y, n, v1, v2, v3));
            ...
            if ((x <= n)) {
                ...
                while ((x <= n) && unroll--) {
                    ...
                    loopcount++;
                    loopcheck(fptr, vars, temp_x, temp_y, temp_n, temp_v1, temp_v2, temp_v3, x, y,
                              n, v1, v2, v3);
                }
            } else {
                // post-check program
                ...
                // post-condition
                if (n > 0) {
                    postcheck(fptr, vars, (y < n), x, y, n, v1, v2, v3)
                }
            }
        }
