...
// variable declarations
    ...
    FILE* inputs = fopen("inputs/inputs_6.txt", "a");
...
#ifndef READ_FROM_FLAG
        long long int v1 = honggBuf[0];
        long long int v2 = honggBuf[1];
        long long int v3 = honggBuf[2];
        long long int x = honggBuf[3];
        long long int size = honggBuf[4];
        long long int y = honggBuf[5];
        long long int z = honggBuf[6];
#endif
        ...
        if (choices > 10000) {
            ...
            if ((x < size)) {
                ...
                while ((x < size) && unroll--) {
                    ...
                    loopcount++;
                    loopcheck(fptr, vars, temp_v1, temp_v2, temp_v3, temp_x, temp_size, temp_y,
                              temp_z, v1, v2, v3, x, size, y, z);
                }
            } else {
                // post-check program
                assume((postflag == 0));
