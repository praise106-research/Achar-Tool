...
def test_loop():
    ...
    for b in range(len(g_list)):
        ...
        node_embedding = node_embedding_batch[
            embedding_offset:embedding_offset + g.pg.num_nodes(), :]
        embedding_offset += g.pg.num_nodes()
        nll_list, value_list, reward_list, root, _ = rollout(g,
                                                             node_embedding,
                                                             decoder,
                                                             use_random=True,
                                                             eps=0.0)
    ...
    if cmd_args.phase == 'test' and len(dataset.test_indices) == len(
            stat_counter.reported):
        sys.exit()
...
if __name__ == '__main__':
    ...
    if cmd_args.init_model_dump is not None and os.path.isfile(
            cmd_args.init_model_dump + '.encoder'):
        encoder.load_state_dict(
            torch.load(cmd_args.init_model_dump + '.encoder'))
        decoder.load_state_dict(
            torch.load(cmd_args.init_model_dump + '.decoder'))
    ...
    for epoch in range(cmd_args.num_epochs):
        ...
        pbar = tqdm(range(100))
        for k in pbar:
            if cmd_args.phase == 'test' and len(stat_counter.reported) == len(
                    dataset.test_indices):
                sys.exit()

            g_list = dataset.sample_minibatch(cmd_args.rl_batchsize,
                                              replacement=True)
        ...
        if cmd_args.save_dir is not None and cmd_args.phase == 'train':
            torch.save(encoder.state_dict(),
                       cmd_args.save_dir + '/epoch-%d.encoder' % (epoch + 1))
            torch.save(decoder.state_dict(),
                       cmd_args.save_dir + '/epoch-%d.decoder' % (epoch + 1))
            torch.save(encoder.state_dict(),
