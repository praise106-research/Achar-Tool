...
class IDecoder(nn.Module):
    ...
    def choose_action(self, state, cls_w, use_random, eps):
        if type(cls_w) is Variable or type(cls_w) is Parameter or type(
                cls_w) is torch.Tensor:
    ...
    def recursive_decode(self, pg_node_list, lv, use_random, eps):
        if lv == 0:  # subtree expression root node, decode the first symbol
            ...
            picked = self.choose_action(self.latent_state,
                                        self.variable_embedding, use_random,
                                        eps)
            self.update_state(
                torch.index_select(self.variable_embedding, 0, picked))
            left_node = ExprNode(pg_node_list[picked.data.gpu()[0]])

            # recursively construct right child
            right_node = self.recursive_decode(pg_node_list, 1, use_random,
                                               eps)
            ...
            if lv < MAX_DEPTH:
                ...
                picked = self.choose_action(self.latent_state, classifier,
                                            use_random, eps)
                ...
                if idx < len(LIST_OP):  # it is an op node
                    ...
                    cur_node.children.append(
                        self.recursive_decode(pg_node_list, lv + 1, use_random,
                                              eps))
                    cur_node.children.append(
                        self.recursive_decode(pg_node_list, lv + 1, use_random,
                                              eps))
                ...
                picked = self.choose_action(self.latent_state,
                                            self.var_const_embedding,
                                            use_random, eps)
                self.update_state(
                    torch.index_select(self.var_const_embedding, 0, picked))
    ...
    def forward(self, env, node_embedding, use_random, eps=0.05):
        ...
        if env.root is None:
            ...
            first_decision = self.choose_action(self.latent_state,
                                                self.decision, use_random, eps)
            act = first_decision.data.cpu()[0]
            if act == 0 or (act == 1 and env.and_budget() == 0) or (
                    act == 2 and env.or_budget() == 0):
        ...
        subexpr_node = self.recursive_decode(env.pg_nodes(), 0, use_random,
                                             eps)
    ...
    def get_init_embedding(self, node_embedding, state):
        if cmd_args.attention:
            ...
            if state is None or allnone:
                ...
                logits = torch.sum(node_embedding * self.latent_state,
                                   dim=1,
                                   keepdim=True)
            weights = F.softmax(logits, dim=0)
            init_embedding = torch.sum(weights * node_embedding,
                                       dim=0,
                                       keepdim=True)
        else:
...
class AssertAwareDecoder(IDecoder):
    ...
    def choose_operand(self, env, node_embedding, use_random, eps):
        ...
        picked = self.choose_action(self.latent_state, selector, use_random,
                                    eps)
    ...
    def recursive_decode(self, env, lv, use_random, eps):
        if lv == 0:  # subtree expression root node, decode the first symbol
            # left child, which is a variable
            left_node = self.choose_operand(env, self.variable_embedding,
                                            use_random, eps)
            ...
            if lv < MAX_DEPTH:
                ...
                if env.core_var_budget(self.cur_token_used + tmp - 1) < 0:
                    ...
                    sp = self.choose_action(self.latent_state,
                                            self.tree_grow_decision,
                                            use_random, eps)
                    decision = to_num(sp)
                ...
                if decision == 0:  # op node
                    ...
                    cur_node = self.choose_operand(env,
                                                   self.var_const_embedding,
                                                   use_random, eps)
                    self.cur_token_used += 1
            else:  # can only choose variable or const
                cur_node = self.choose_operand(env, self.var_const_embedding,
                                               use_random, eps)
...
class InvariantTreeNode(ExprNode):
    ...
    def __repr__(self):
        if self.name == "" and self.rule is not None:
            if len(self.children) == 0:
                ...
                return " ".join([
                    "(" + str(x) + ")" if len(x.children) > 0 else str(x)
    ...
    def expr_str(self):
        if self.name == "" and self.rule is not None:
            if len(self.children) != 0:
                tmp_list = [
                    " ( " + x.expr_str() + " ) " if len(x.children) > 0
                    and x.expr_str() != "" else x.expr_str()
                    for x in self.children
                ]
                expr_list = []
                removed_bin = False
                for i in range(len(tmp_list) - 1, -1, -1):
                    if tmp_list[i] == "":
                        ...
                        pass
                    elif tmp_list[i] in ("&&", "||") and all(
                            x == ")" for x in expr_list):
                        pass
                    elif (tmp_list[i] in Z3_OP or
                          tmp_list[i] in Z3_CMP) and all(x == ")"
                                                         for x in expr_list):
                        removed_bin = True
...
class GeneralDecoder(AssertAwareDecoder):
    ...
    def forward(self, env, node_embedding, use_random, eps=0.05):
        ...
        self.var_const_embedding = node_embedding[0:env.num_vars() +
                                                  env.num_consts(), :]
        self.const_embedding = node_embedding[env.num_vars():env.num_vars() +
                                              env.num_consts(), :]

        self.root = self.updateTreeWRTNode(env, self.root, node_embedding,
                                           use_random, eps)

        return None, self.root, self.nll, self.est, self.latent_state

    def updateTreeWRTNode(self,
                          env,
                          node,
                          node_embedding,
                          use_random,
        ...
        if node.name == "" and node.rule is not None and len(
                node.children) == 0 and node.rule != "var":
            s = 0
            ...
            if node.name in ("", "var", "const"):
                return self.updateTreeWRTNode(env, node, node_embedding,
                                              use_random, eps, depth + 1)
            else:
                return node
            ...
            node = self.choose_var(env, self.variable_embedding, use_random,
                                   eps, var_list)
            node.rule = "var"
            return node
        elif node.name == "var":
            ...
            for i in range(len(node.children)):
                if node.children[i].name == "const" or node.children[
                        i].name == "var" or (node.children[i].name == "" and
                                             node.children[i].rule == "var"):
                    node.children[i] = self.updateTreeWRTNode(
                        env, node.children[i], node_embedding, use_random, eps,
                        depth + 1)
            ...
            const_picked = env.num_vars() + self.choose_action(
                self.latent_state, self.const_embedding, True, 0.05)
            self.update_state(
                torch.index_select(self.var_const_embedding, 0, const_picked))
