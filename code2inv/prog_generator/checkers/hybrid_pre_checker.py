...
# from code2inv.prog_generator.checkers.c_inv_checker_all import inv_solver as z3_inv_solver
...
filepath = os.path.join(pwd, os.pardir, f"HYBRID_PRE/include/{example}.h")
fuzzbase = os.path.join(pwd, os.pardir, f"HYBRID_PRE")
...
def call_fuzzsolver(timeout):
    ...
    try:
        ...
        output = run(
            f"timeout {timeout} {fuzzbase}/fuzz.sh -b {fuzzbase}/bin -t {fuzzbase}/tests \
...
def dump_template(file, inv_code):
    with open(file, mode="w") as fileptr:
        fileptr.write(f"#define EXECUTE_PRE 1 // generated by code2inv\n")
        # fileptr.write(f"#define EXECUTE_LOOP 1 // generated by code2inv\n")
        # fileptr.write(f"#define EXECUTE_POST 1 // generated by code2inv\n")
...
def inv_solver(vc_file: str, inv: str):
    ...
    tqdm.write(f"[Hybrid PRE Checker] {inv}")

    if not os.path.isdir("hybrid_pre_calls"):
        os.mkdir("hybrid_pre_calls")
    ...
    if choice == 1:
        tqdm.write("[Hybrid PRE Checker] (1) Z3 Called \n")

        for i in range(len(collection_semantic)):
            collection_semantic[i] = None

        with open(f"hybrid_pre_calls/call_logs_{example}.log",
        ...
        # tqdm.write(f"[Hybrid PRE Checker] {z3_res}")
        ...
        if all(x is None for x in z3_res):
            ...
            tqdm.write("[Hybrid PRE Checker] Z3 is UNSAT, Calling fuzzer \n")
            ...
            # with open(f"hybrid_pre_calls/call_logs_{example}.log", mode="a") as call_logger:
            #     call_logger.write(
            #         f"[{datetime.datetime.now()}] Z3 : {z3_res}\n")

            return z3_res
    else:
        tqdm.write("[Hybrid PRE Checker] (2) Fuzzer Only \n")
