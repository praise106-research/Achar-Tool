    ...
    while True:
        for idx in range(len(old_list)):
            if old_list[idx] in un_op_list:
                if idx == 0 or old_list[idx -
                                        1] in op_list or old_list[idx -
                                                                  1] == "(":
...
def inv_checker(vc_file: str, inv: str, assignments):
    ...
    for token in inv_tokenized:
        if token[
                0] in "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz" and token not in (
                    "and",
                    "or",
                ):
            var_list.add(token)
    ...
    with open(
            os.path.join(
                "candidates",
                f"candidates_{cmd_args.example}_{cmd_args.spec_type}_{cmd_args.afl_timeout}_{cmd_args.num_epochs}.log"
            ),
            mode="a",
    ) as file:
        file.write(f"{inv}\n")
...
def inv_solver(vc_file: str, inv: str):
    ...
    for a in t:
        if a.string != "":
            inv_tokenized.append(a.string)
    inv = stringify_prefix_stack(
        postfix_prefix(infix_postfix(condense(inv_tokenized))))
    inv = inv.replace("==", "=", -1)
    ...
    for i in range(3):
        s = tpl[0] + inv + tpl[i + 1]
        # with open(f"query_{random.randint(0, 1000)}.smt2", mode="w") as smt_writer:
        #     smt_writer.write(s)
        sol.reset()
        try:
    ...
    with open(
            os.path.join(
                "models",
                f"z3_{cmd_args.spec_type}_model_{cmd_args.example}.log"),
            mode="a",
    ) as file:
        file.write(f"{res}\n")
