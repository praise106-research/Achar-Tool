...
example = f"hybrid_{example}"
tqdm.write(f"{example}\n")
dump_results = os.path.join(pwd, os.pardir, f"results/log_inv_{example}.txt")
filepath = os.path.join(pwd, os.pardir, f"Hybrid_Examples/include/{example}.h")
fuzzbase = os.path.join(pwd, os.pardir, f"Hybrid_Examples")
...
def inv_checker(vc_file: str, inv: str, assignments):
    # tqdm.write(f"Checking RQ5 {inv}")
    # TODO : Make a checker that will interpret and return the results.
    ret_vals = z3_inv_checker(vc_file, inv, assignments)
    # tqdm.write(f"[RQ5 Check] {ret_vals}")
    return ret_vals
