...
void precheck(FILE* fptr, char* modelFileBuffer, long long int x, long long int y, long long int a,
    ...
    int f = preflag;
    setflag(INV(x, y, a, b, res), preflag);
    if (f == 0 && preflag == 1) {
...
void loopcheck(FILE* fptr, char* modelFileBuffer, long long int temp_x, long long int temp_y,
               long long int temp_a, long long int temp_b, long long int temp_res, long long int x,
               long long int y, long long int a, long long int b, long long int res) {
    int f = loopflag;
    setflag(INV(x, y, a, b, res), loopflag);
    if (f == 0 && loopflag == 1) {
        fprintf(fptr, "LoopStart : %s : %lld, %s : %lld, %s : %lld, %s : %lld, %s : %lld\n", "x",
                temp_x, "y", temp_y, "a", temp_a, "b", temp_b, "res", temp_res);
...
#define postcheck(fptr, modelFileBuffer, cond, x, y, a, b, res) \
    \ 
{                                                          \
        \ 
    int f = postflag;                                           \
...
// variable declarations
    ...
    for (;;) {
        ...
        if (choices > 10000) {
            ...
            // fprintf(fptr, "Pre\n");
            // precheck
            precheck(fptr, vars, x, y, a, b, res);
        } else {
            // loop-check program
            assume((loopflag + postflag < 2));
            assume(INV(x, y, a, b, res));
            ...
            if (b > 0) {
                ...
                while ((b > 0) && unroll--) {
                    ...
                    loopcount++;
                    loopcheck(fptr, vars, temp_x, temp_y, temp_a, temp_b, temp_res, x, y, a, b,
                              res);
                }
            } else {
                // post-check program
                ...
                // fprintf(fptr, "Post\n");
                // post-condition
                postcheck(fptr, vars, (res == binpow(x, y)), x, y, a, b, res)
            }
        }
