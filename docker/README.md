...
[![asciicast](https://asciinema.org/a/465317.png)](https://asciinema.org/a/465317)
...
```bash
# from code2inv base dir/
$ docker run --name code2inv --cpus=4 --memory=24g -it -v $PWD:/project code2inv/code2inv:latest

# Create all the SSA JSON Graphs for the benchmark programs
$ cd /project
$ ./generate.sh
```
...
The `-v` flag is used to mount a volume into the docker container. In this case, the `pwd` is mounted by the name `current`. 
