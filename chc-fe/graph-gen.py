    ...
    for rule in db.get_rules():
        ...
        for pred in rule.body():
            ...
            if pred_name == head_name:
                inv_fun.append(
                    (pred_name,
                     [str(x)[:str(x).rfind("_")] for x in pred.children()]))
...
class CHCGraph:
    ...
    def add_quantified_node(self, quant_expr: QuantifierRef):
        ...
        if quant_expr.body().decl().name() == "Implies":
            body_node, _, _ = self.add_rule_nodes(
                quant_expr.body().children()[0], var_node_mapping, quant_expr,
                {}, {})
            # self.nodes.append(body_node)
            flow = (node.id, body_node.id)
            if flow not in self.control_flow:
                self.control_flow.append(flow)
            head_node, _, _ = self.add_rule_nodes(
                quant_expr.body().children()[1], var_node_mapping, quant_expr,
                {}, {})
            ...
            body_node, _, _ = self.add_rule_nodes(quant_expr.body(),
                                                  var_node_mapping, quant_expr,
                                                  {}, {})
            # self.nodes.append(body_node)
            flow = (node.id, body_node.id)
            if flow not in self.control_flow:
    ...
    def add_rule_nodes(self,
        ...
        if len(expr.children()) == 0:
            ...
            if cmd_str not in cmd_mapping:
                ...
                self.nodes.append(node)
                for child in expr.children():
                    child_node, const_mapping, cmd_mapping = self.add_rule_nodes(
                        child, var_node_mapping, quant_expr, const_mapping,
                        cmd_mapping)
    ...
    def to_json(self):
        ...
        for flow_idx in range(len(self.control_flow)):
            if flow_idx > 0:
                json_str += ',\n'
            json_str += '\t\t["' + self.control_flow[flow_idx][
                0] + '", "' + self.control_flow[flow_idx][1] + '"]'
        json_str += '\n\t]\n}'
        return json_str
